...
> 模型在长对话中逐渐遗忘初始约束。三层方案，从复制粘贴到生产级状态机。
...
它是结构性的信息损失现象：封闭生成循环中，每轮复制都在降低原始信号的保真度。模型不是"忘了"某条指令——它丢失了那个告诉它哪些指令重要的认识框架。

本项目把症状当作热力学事实处理。

解法不是微调模型，而是在封闭系统上 engineered 可控的开口。
...
若山照彦。山梨大学先进生物技术中心。连续小鼠克隆。第 58 代。DNA 残缺，或表达出错。不育。

只有引入外部基因组，谱系才得以恢复。

是同一条曲线。

大模型每轮生成是一次复制。训练数据是对世界的有损采样。不可逆。
